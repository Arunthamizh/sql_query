...
/* Ex: to remove the table from the database*/
...
/* JOIN QUERIES:
1.Inner Join(To select common values only) form both table*/

-- The INNER JOIN is a process that matches rows from the first table and the second table
-- which have the same key (as defined by the ON constraint) to create a result row with the combined columns from both tables.
...
-- The RIGHT JOIN is the same, but reversed, keeping rows in B regardless of whether a match is found in A. Finally,
...
SELECT name,city,(
  ...
  WHEN city='Namakkal' THEN 200
  WHEN city='Chennai' THEN 300
  WHEN city='Hosur' THEN 400
  ELSE 0
