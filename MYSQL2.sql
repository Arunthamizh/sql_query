...
FROM students
INNER JOIN course ON students.cid=course.cid
INNER JOIN marks ON students.id=marks.id WHERE course.cname='BCA'
having Result = 'PASS' AND (Average >= 70 AND Average <= 100);

/* 'having' is the keyword using get the value for the column that are generated in the view that are
...
/*save the select as table but it was stored as view we do all the things like table*/
